// Cross-DM stop_playtest signaling via plugin:SetSetting. Play-mode tests run
// in a separate DataModel; cross-DM LogService.MessageOut reflection does NOT
// reach it, and StudioTestService:EndTest is illegal from the edit DM. The
// plugin settings store is the one channel shared across every DataModel the
// plugin runs in (edit DMs, play-server DMs, play-client DMs).
//
// For each place we use a dedicated key "MCP_STOP_PLAY_<instanceId>" as a tiny
// tokenized request/result mailbox:
//
//   * The edit DM's handler writes a stop request token into its own key
...
//
// The per-place key scoping matters because plugin settings are shared across
// every Studio window running the plugin — a shared boolean would let one
// window's stop kill another window's playtest. Tokens (vs booleans) prevent
// a stale flag from a crashed session killing the next playtest, and
// flag-cleared-equals-success false positives.
//
// During publish-after-connect, both "anon:<uuid>" and "place:<PlaceId>" can
// refer to the same Studio place, so stop requests are mirrored across both
// keys while the monitor waits for a matching result on either key.
...
// Must be called in the EDIT DM before a playtest starts. For unpublished
// places (PlaceId 0) it stamps the __MCPPlaceId attribute so the play-DM clone
// inherits the SAME anon id — otherwise the edit DM and the play-server DM mint
// two different GUIDs, write different mailbox keys, and stop_playtest can never
// be confirmed for the first play session of a fresh place.
function ensurePlaceId(): void {
	computeInstanceIds();
}

// Both the edit DM and the play-server DM must agree on the place identifier.
// Published places: placeId. Unpublished: UUID stored as an attribute on
// ServerStorage, which travels with the place clone into the play DM.
